// Use conda environment CUDA headers instead of system headers
...
void TVMNeuralNet::predict_gpu(float* d_input, float* d_output, cudaStream_t stream) {
    ...
    if (stream) {
        auto stream_func = tvm::ffi::Function::GetGlobal("tvm.contrib.cuda.set_stream");
        if (stream_func.has_value()) {
            // Convert cudaStream_t to void* to avoid type system issues
            void* stream_ptr = static_cast<void*>(stream);
            (*stream_func)(static_cast<int>(device_id_), stream_ptr);
        }
    }
...
extern "C" void example_integration_workflow() {
    ...
    float d_dist_results[4] = {1.0, -0.5, -0.2, 0.3};
    float d_nn_output[4];
    // Run neural network
